...
export function MathCaptcha({ onValidate }: MathCaptchaProps) {
  ...
  return (
    <div className="space-y-1.5">
      <label className={`text-[11px] font-black text-slate-400 ${language === 'ar' ? 'mr-1' : 'ml-1'} uppercase tracking-[0.2em] flex justify-between`}>
        <span>{t('captcha_label')}</span>
        {userAnswer && (
          userAnswer !== '' && parseInt(userAnswer, 10) === num1 + num2
            ? <span className="text-green-500">{t('captcha_valid')}</span>
            : <span className="text-red-500">{t('captcha_incorrect')}</span>
        )}
      </label>
      <div className={`relative group/captcha w-full bg-slate-50/80 focus-within:bg-white rounded-full py-2 px-2 transition-all border shadow-sm flex items-center justify-between ${language === 'ar' ? 'pr-4 pl-2 text-right' : 'pl-4 pr-2 text-left'} ${
          userAnswer && parseInt(userAnswer, 10) === num1 + num2 
        ...
        <div className={`flex items-center ${language === 'ar' ? 'space-x-reverse space-x-3' : 'space-x-3'}`}>
          ...
          <button
            ...
            title={t('captcha_refresh_title')}
          >
            <RefreshCw size={18} />
          </button>
        </div>
        <div className={language === 'ar' ? 'pl-1' : 'pr-1'}>
          <input
            type="number"
            value={userAnswer}
