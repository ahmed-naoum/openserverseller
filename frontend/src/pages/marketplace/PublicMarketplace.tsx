...
export default function PublicMarketplace() {
  ...
  const handleCustomSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!customName.trim() || !customCategory.trim() || !customDescription.trim() || customQty < 1) {
    ...
    try {
      ...
      toast.success('Votre demande de produit personnalisé a été soumise avec succès.');
          ...
          <aside className="hidden lg:block w-72 flex-shrink-0">
            <div className="sticky top-28 bento-card border border-white/60 bg-white/70 backdrop-blur-2xl p-8 space-y-8 shadow-[0_20px_40px_-15px_rgba(0,0,0,0.05)] rounded-[2.5rem]">
               ...
               <div className="mt-8 pt-8 border-t border-slate-200">
                  <div className="relative overflow-hidden bg-[#0A0A0A] rounded-3xl p-6 border border-white/10 shadow-2xl group" dir={language === 'ar' ? 'rtl' : 'ltr'}>
                     ...
                     <div className={`relative z-10 ${language === 'ar' ? 'text-right' : 'text-left'}`}>
                       <p className="text-[10px] font-black text-white/50 uppercase tracking-[0.2em] mb-3 flex items-center gap-2 justify-start">
                         <Sparkles size={12} className="text-amber-400"/> 
                         {language === 'ar' ? 'منتج مخصص' : language === 'en' ? 'Custom Product' : 'Produit Personnalisé'}
                       </p>
                       <p className="text-xs font-black text-white leading-snug tracking-tight mb-5">
                         {language === 'ar' 
                       ...
                       <button 
                         ...
                         className="w-full bg-white text-black py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-gray-100 hover:scale-105 transition-all shadow-xl shadow-white/10"
                       >
