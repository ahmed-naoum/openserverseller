...
/**
 * This page is the "Non validé" queue and nothing else — a cart already
 * converted into a lead is not work, so it never appears here. Every badge on
 * the page therefore counts that queue, not the whole visible set.
 */
interface Counts {
  unsaved: number;
  unsavedComplete: number;
  unsavedIncomplete: number;
}
...
export default function AbandonedCarts() {
  ...
  const [counts, setCounts] = useState<Counts>({
    unsaved: 0, unsavedComplete: 0, unsavedIncomplete: 0,
  });

  // Carts whose replay the agent has opened, so the button reads "Revu".
  ...
  const load = useCallback(async (p = page, q = search, pq = phoneFilter) => {
    setLoading(true);
    try {
      const r = await leadsApi.abandonedCarts({
        // Hard-locked to the work queue: carts already converted into a lead are
        // filtered out server-side, so they never reach this page.
        page: p, limit: 30, search: q || undefined, status: 'unsaved', phoneQuality: pq,
      });
  ...
  useEffect(() => { load(1, '', 'all'); /* eslint-disable-next-line */ }, []);
  ...
  const onSearchChange = (value: string) => {
    ...
    searchDebounceRef.current = setTimeout(() => load(1, value, phoneFilter), 400);
  };
  ...
  const convert = async (cart: Cart) => {
    ...
    try {
      ...
      const incomplete = checkPhoneCompleteness(cart.phone).quality === 'incomplete';
      ...
      // The cart leaves the page for good — once recovered it is a lead, and this
      // page only ever lists what is still to be recovered.
      setCarts(prev => prev.filter(c => c.id !== cart.id));
      setTotal(t => Math.max(0, t - 1));
      setCounts(prev => ({
        unsaved: Math.max(0, prev.unsaved - 1),
        unsavedComplete: Math.max(0, prev.unsavedComplete - (incomplete ? 0 : 1)),
        unsavedIncomplete: Math.max(0, prev.unsavedIncomplete - (incomplete ? 1 : 0)),
      }));
    } catch (err: any) {
    ...
    <div className="p-2 sm:p-6 max-w-[1200px] mx-auto space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-4 sm:p-6 rounded-3xl border border-gray-100 shadow-xs">
        <div className="flex items-center gap-3 sm:gap-4 min-w-0">
          <div className="w-11 h-11 sm:w-14 sm:h-14 rounded-2xl bg-orange-500/10 text-orange-600 flex items-center justify-center shrink-0">
            <ShoppingCart className="w-6 h-6 sm:w-7 sm:h-7" />
          </div>
          <div className="min-w-0">
            <h1 className="text-xl sm:text-2xl font-black text-gray-900">Paniers Abandonnés</h1>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <div className="relative flex-1 min-w-0 md:flex-none">
            ...
            <input
              ...
              className="w-full min-w-0 pl-9 pr-3 py-2 bg-gray-50 border border-gray-200 rounded-xl text-xs font-medium focus:outline-none focus:ring-2 focus:ring-orange-500/20"
            />
          </div>
          <button onClick={() => load(1)} className="shrink-0 px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs font-bold rounded-xl flex items-center gap-2 transition-all">
            <RefreshCw className="w-3.5 h-3.5" /> Actualiser
          </button>
        </div>
      </div>

      {/* One filter axis left — how dialable the number is. The status axis is
          gone: this page is the "Non validé" queue by definition, so there is
      ...
      <div className="bg-white rounded-3xl border border-gray-100 shadow-xs p-4 space-y-3">
        <div className="flex items-start gap-x-8 gap-y-3 flex-wrap">
          <div className="space-y-1.5">
            ...
            <span
              title="Cette page ne liste que les paniers non validés. Un panier converti en lead en sort définitivement."
              className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-2xl text-[11px] font-black bg-red-500 text-white shadow-xs"
            >
              <AlertTriangle className="w-3 h-3" /> Non validé ({counts.unsaved})
            </span>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="inline-flex items-center bg-gray-50 border border-gray-200 rounded-2xl p-1">
              {([
                { key: 'all', label: 'Tous', count: counts.unsaved, icon: null, active: 'bg-gray-900 text-white' },
                { key: 'complete', label: 'N° complet', count: counts.unsavedComplete, icon: Phone, active: 'bg-emerald-500 text-white' },
                { key: 'incomplete', label: 'N° incomplet', count: counts.unsavedIncomplete, icon: PhoneOff, active: 'bg-amber-500 text-white' },
              ] as const).map((tab) => {
                const Icon = tab.icon;
                return (
                  <button
          ...
          <div className="ml-auto flex items-center gap-2 flex-wrap self-center">
            <span className="inline-flex items-center gap-2 px-4 py-2 rounded-2xl text-sm font-black border bg-red-50 text-red-600 border-red-100">
              <AlertTriangle className="w-4 h-4" />
              {total} {total === 1 ? 'panier' : 'paniers'} à rappeler
            </span>
            {!scoped && (
                        ...
                        <div key={c.id} className="flex items-center gap-4 px-5 py-4 hover:bg-orange-50/30 transition-colors flex-wrap">
                          ...
                          <div className="flex-1 min-w-[180px]">
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              {/* Carts whose number is already a lead — converted
                                  from here or not — never reach this page, so
                                  every row carries the same state. */}
                              <span
                          ...
                          {/* The number is the point of the page: masked only while
                              the cart is on cooldown, then shown ready to dial. */}
                          {phone.quality === 'incomplete' ? (
                            <span
                              title={phone.reason || 'Numéro incomplet'}
