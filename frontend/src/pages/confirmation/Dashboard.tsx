...
import { 
  ...
  X as CloseIcon, MapPin, Globe, CreditCard, ChevronRight, Activity, Search
} from 'lucide-react';
import { 
                         ...
                         {selectedUser.kycDocuments.map((doc, idx) => (
                           <div key={idx} className="bg-gray-800 rounded-2xl overflow-hidden shadow-2xl border border-gray-700">
                              <div className="bg-gray-900 px-4 py-3 border-b border-gray-700 flex justify-between items-center">
                                 ...
                                 <a href={doc.documentUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-teal-400 hover:text-teal-300 flex items-center gap-1 font-medium bg-teal-500/10 px-3 py-1.5 rounded-lg transition-colors">
                  ...
                  <div className="flex-1 overflow-y-auto p-6 space-y-8 font-inter">
                      ...
                      <div className="flex items-start gap-4">
                        <div className={`w-16 h-16 rounded-2xl flex items-center justify-center text-2xl font-black text-white shadow-lg ${
                          selectedUser.role.name === 'VENDOR' ? 'bg-gradient-to-br from-purple-500 to-indigo-600' :
                          selectedUser.role.name === 'INFLUENCER' ? 'bg-gradient-to-br from-pink-500 to-rose-600' : 
                          'bg-gradient-to-br from-blue-500 to-cyan-600'
                        }`}>
                          {selectedUser.profile?.fullName?.charAt(0).toUpperCase() || '?'}
                        </div>
                        <div>
                          <h3 className="text-2xl font-black text-gray-900 tracking-tight">{selectedUser.profile?.fullName || 'Anonyme'}</h3>
                          <div className="flex flex-wrap gap-2 mt-2">
                            <span className="px-2.5 py-1 bg-gray-100 text-gray-700 text-xs font-bold rounded-md">{selectedUser.role?.name}</span>
                            <span className="px-2.5 py-1 bg-gray-100 text-gray-700 text-xs font-bold rounded-md flex items-center gap-1"><Clock className="w-3 h-3"/> {new Date(selectedUser.createdAt).toLocaleDateString()}</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="bg-gray-50 rounded-2xl p-5 border border-gray-100 space-y-4">
                        <div>
                          ...
                          <p className="font-medium text-gray-900 flex justify-between items-center">
                            {selectedUser.email || <span className="text-gray-400 italic">Non fourni</span>}
                          </p>
                        </div>
                        <div>
                          <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Téléphone</p>
                          <p className="font-medium text-gray-900">{selectedUser.phone || <span className="text-gray-400 italic">Non fourni</span>}</p>
                        </div>
                      </div>

                      {/* Socials (If Influencer) */}
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            {selectedUser.profile?.instagramUsername && (
                              <a href={`https://instagram.com/${selectedUser.profile.instagramUsername}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 p-3 rounded-xl bg-pink-50 border border-pink-100 text-pink-700 hover:bg-pink-100 transition-colors">
                                <FaInstagram className="text-lg" /> <span className="font-bold text-sm truncate">@{selectedUser.profile.instagramUsername}</span>
                              </a>
                            )}
                            {selectedUser.profile?.tiktokUsername && (
                              <a href={`https://tiktok.com/@${selectedUser.profile.tiktokUsername}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 p-3 rounded-xl bg-gray-100 border border-gray-200 text-gray-900 hover:bg-gray-200 transition-colors">
                                <FaTiktok className="text-lg" /> <span className="font-bold text-sm truncate">@{selectedUser.profile.tiktokUsername}</span>
                              </a>
                            )}
                            {/* Add logic for others if needed */}
                            {!selectedUser.profile?.instagramUsername && !selectedUser.profile?.tiktokUsername && (
                              <p className="text-sm text-gray-500 italic col-span-2">Aucun réseau renseigné</p>
                            )}
                          </div>
                        </div>
                      )}
                      ...
                      <div>
                          <h4 className="text-sm font-black text-gray-900 uppercase tracking-wider mb-4 flex items-center gap-2">
                            <MapPin className="w-4 h-4 text-gray-400" /> Localisation & Securité
                          </h4>
                          <div className="space-y-3">
                            <div className="flex justify-between items-center py-2 border-b border-gray-50">
                              <span className="text-sm text-gray-500 font-medium">Ville déclarée</span>
                              <span className="text-sm font-bold text-gray-900">{selectedUser.profile?.city || '-'}</span>
                            </div>
                            <div className="flex justify-between items-center py-2 border-b border-gray-50">
                            ...
                            <div className="flex flex-col gap-1 py-2">
                              <span className="text-sm text-gray-500 font-medium">Adresse</span>
                      ...
                      {selectedUser.kycStatus === 'PENDING' ? (
                        <div className="flex flex-col gap-3">
                          <button
                            ...
                            disabled={isUpdating}
                            className="w-full relative group overflow-hidden flex items-center justify-center gap-2 py-4 bg-teal-500 text-white rounded-2xl font-black text-lg transition-all hover:bg-teal-400 hover:shadow-xl hover:shadow-teal-500/30 disabled:opacity-50"
                          >
                            ...
                            <CheckCircle className="w-6 h-6 relative z-10" /> <span className="relative z-10">Valider & Approuver</span>
                          </button>
                          <button
                            onClick={() => {
                        ...
                        <div className={`flex items-center justify-center gap-3 p-5 rounded-2xl border-2 ${
                          selectedUser.kycStatus === 'APPROVED' ? 'bg-green-50 border-green-200 text-green-700' : 'bg-red-50 border-red-200 text-red-700'
                        }`}>
                          {selectedUser.kycStatus === 'APPROVED' ? <CheckCircle className="w-8 h-8" /> : <XCircle className="w-8 h-8" />}
                          <div>
                              <p className="font-black text-lg leading-tight">{selectedUser.kycStatus === 'APPROVED' ? 'Dossier Approuvé' : 'Dossier Rejeté'}</p>
                              <p className="text-sm font-medium opacity-80">Traitements clos pour cet utilisateur.</p>
                          </div>
                        </div>
                      )}
                  </div>
